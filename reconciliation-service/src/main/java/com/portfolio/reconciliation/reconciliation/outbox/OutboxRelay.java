...
/**
 * Relay do Transactional Outbox (ADR-0006), duplicado do ingestion-service por rule of three
 * (ADR-0010): lê as linhas pendentes com {@code SKIP LOCKED}, publica no RabbitMQ e marca {@code
 * published_at} apenas após o confirm do broker. Um purge agendado remove as publicadas antigas.
 */
@Component
public class OutboxRelay {
  ...
  public OutboxRelay(
      ...
      @Value("${reconciliation.outbox.relay.max-attempts}") int maxAttempts,
      @Value("${reconciliation.outbox.relay.batch-size}") int batchSize,
      @Value("${reconciliation.outbox.relay.confirm-timeout-ms}") long confirmTimeoutMs,
      @Value("${reconciliation.outbox.purge.retention-days}") int retentionDays) {
  ...
  @Scheduled(fixedDelayString = "${reconciliation.outbox.relay.interval-ms}")
  @Transactional
  ...
  @Scheduled(fixedDelayString = "${reconciliation.outbox.purge.interval-ms}")
  @Transactional
